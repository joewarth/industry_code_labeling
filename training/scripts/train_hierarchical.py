...
def apply_mask(logits, allowed_mask):
    # allowed_mask shape: (batch, n_classes), dtype bool
...
def main():
    ...
    print("got project_dir", flush=True)
    ...
    y_train = load_npz_dict(processed_dir / "y_train.npz")
    y_valid = load_npz_dict(processed_dir / "y_valid.npz")
    ...
    print("computed dimensions", flush=True)

    train_ds = HierDataset(X_train, y_train)
    valid_ds = HierDataset(X_valid, y_valid)
    ...
    print("built dataloaders", flush=True)
    ...
    print("built optimizer and loss", flush=True)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if valid_metrics["acc_y6"] > best_valid_acc:
            ...
            epochs_without_improvement = 0
            torch.save(model.state_dict(), models_dir / "hierarchical_cnn_best.pt")
    ...
    model.load_state_dict(torch.load(models_dir / "hierarchical_cnn_best.pt", map_location=DEVICE))
    ...
    with open(models_dir / "hierarchical_cnn_history.json", "w") as f:
        json.dump(history, f, indent=2)

    with open(models_dir / "hierarchical_cnn_test_metrics.json", "w") as f:
    ...
    with open(models_dir / "hierarchical_cnn_config.json", "w") as f:
